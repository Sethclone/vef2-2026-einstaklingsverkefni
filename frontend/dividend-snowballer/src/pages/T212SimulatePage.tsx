...
import type { SimulationParams, T212SimulationResult } from '../types'
...
export default function T212SimulatePage() {
  const [params, setParams] = useState<SimulationParams>(DEFAULT_PARAMS)
  const [result, setResult] = useState<T212SimulationResult | null>(null)
  ...
  const handleRun = async (e: React.FormEvent) => {
    ...
    try {
      setRunning(true)
      const data = await api.t212.simulate(params)
      setResult(data)
    } catch (err) {
    ...
    <div className="page">
      <div className="page-header">
        <div>
          <div className="breadcrumb">
            <Link to="/">My Portfolio</Link> / Simulate
          </div>
          <h1>Snowball Simulator</h1>
          <p className="text-muted">Project your T212 holdings with dividend reinvestment and withholding tax.</p>
        </div>
      </div>
      ...
      {result && (
        <div className="alert alert-info">
          Dividend withholding tax applied: <strong>{result.dividendTaxRate.toFixed(2)}%</strong> weighted average
          across all holdings. After-tax dividends are used for DRIP reinvestment.
        </div>
      )}
        ...
        <div className="sim-results">
          ...
          {result && (
            <>
              ...
              {/* Annual Dividends Chart (after-tax) */}
              <div className="card chart-card">
                <h3>Annual Dividends (after tax)</h3>
